...
async def upload_quality_report(
    teacher_id: int = Form(...), month: int = Form(...), year: int = Form(...), file: UploadFile = File(...)
):
    ...
    try:
        ...
        def find_column(df, aliases):
            ...
            for alias in aliases:
                if normalize(alias) in norm_map:
                    return norm_map[normalize(alias)]
            return None
        ...
        if report_type == "quality":
            ...
            summary_val = str(df[summary_col].iloc[0]) if summary_col else None
            db.add(models.QualityReport(teacher_id=teacher_id, month=month, year=year,
                                        quality_score=score_val, quality_summary=summary_val))
            ...
            summary_val = str(df[summary_col].iloc[0]) if summary_col else None
            db.add(models.StudentReport(teacher_id=teacher_id, month=month, year=year,
                                        student_feedback_score=score_val, student_feedback_summary=summary_val))
            ...
            attitude_val = str(df[attitude_col].iloc[0]) if attitude_col else None
            db.add(models.OperationsReport(teacher_id=teacher_id, month=month, year=year,
                                           operations_on_schedule=schedule_val, operations_attitude_summary=attitude_val))

        db.commit()
        return {"success": True, "message": f"Successfully processed {report_type} report for teacher {teacher_id}",
                "teacher_id": teacher_id, "month": month, "year": year, "report_type": report_type}
...
def get_upload_history(teacher_id: int, db: Session = Depends(get_db)):
    ...
    if not teacher:
        raise HTTPException(status_code=404, detail="Teacher not found")
    history = []
    for report in db.query(models.QualityReport).filter(models.QualityReport.teacher_id == teacher_id).all():
        history.append({"id": report.id, "teacher_name": teacher.name, "report_type": "quality",
                        "month": report.month, "year": report.year, "uploaded_at": report.uploaded_at})
    for report in db.query(models.StudentReport).filter(models.StudentReport.teacher_id == teacher_id).all():
        history.append({"id": report.id, "teacher_name": teacher.name, "report_type": "student",
                        "month": report.month, "year": report.year, "uploaded_at": report.uploaded_at})
    for report in db.query(models.OperationsReport).filter(models.OperationsReport.teacher_id == teacher_id).all():
        history.append({"id": report.id, "teacher_name": teacher.name, "report_type": "operations",
                        "month": report.month, "year": report.year, "uploaded_at": report.uploaded_at})
...
def get_dashboard_data(teacher_id: int, month: int, year: int, report_types: str, db: Session = Depends(get_db)):
    teacher = db.query(models.Teacher).filter(models.Teacher.id == teacher_id).first()
    if not teacher:
        raise HTTPException(status_code=404, detail="Teacher not found")

    requested_types = [t.strip() for t in report_types.split(",")]
    dashboard_data = {"teacher_name": teacher.name, "month": month, "year": year, "monthly_stats": None, "reports": {}}
    ...
    if monthly_stats:
        dashboard_data["monthly_stats"] = {"video_views": monthly_stats.video_views, "bandwidth_gb": monthly_stats.bandwidth_gb}

    if "quality" in requested_types:
        r = db.query(models.QualityReport).filter(
            models.QualityReport.teacher_id == teacher_id,
            models.QualityReport.month == month, models.QualityReport.year == year
        ).first()
        if r:
            dashboard_data["reports"]["quality"] = {"score": r.quality_score, "summary": r.quality_summary, "uploaded_at": r.uploaded_at}

    if "student" in requested_types:
        r = db.query(models.StudentReport).filter(
            models.StudentReport.teacher_id == teacher_id,
            models.StudentReport.month == month, models.StudentReport.year == year
        ).first()
        if r:
            dashboard_data["reports"]["student"] = {"score": r.student_feedback_score, "summary": r.student_feedback_summary, "uploaded_at": r.uploaded_at}

    if "operations" in requested_types:
        r = db.query(models.OperationsReport).filter(
            models.OperationsReport.teacher_id == teacher_id,
            models.OperationsReport.month == month, models.OperationsReport.year == year
        ).first()
        if r:
            dashboard_data["reports"]["operations"] = {"on_schedule": r.operations_on_schedule,
                                                        "attitude_summary": r.operations_attitude_summary, "uploaded_at": r.uploaded_at}
